...
<%
    ...
    request.getSession().setAttribute("prevURL", "manager_serv");
%>
<ul class="hor">
    <li><a class="home" href="/delivery_war/manager_serv"><%=bundle.getString("manager.home")%></a></li>
    <li><a href="/delivery_war/manager/manager_all_serv"><%=bundle.getString("manager.all")%></a></li>
    <li style="float: right"><a href="/delivery_war/logout"><%=bundle.getString("ui.logout")%></a></li>
    <li style="float: right; color: white; margin-right: 10px"><%=bundle.getString("manager.access")%></li>
    <li>
...
<form class="filter" action="manager_serv" method="post">
    <ul>
        ...
        <tr>
            ...
            <td>${bid.totalPrice}
                <c:if test="${bid.status.ordinal == '0'}">
                    <form style="margin-bottom: 2px" action="manager_serv" method="post">
                        <input type="hidden" name="validate" value="${bid.id}">
                        <input type="submit" value="<%=bundle.getString("manager.validate")%>">
                    </form>
                </c:if>
                <form action="manager_serv" method="post">
                    <input type="hidden" name="cancel" value="${bid.id}">
                    <input type="submit" value="<%=bundle.getString("manager.cancel")%>">
                </form>
            </td>
        </tr>
    </c:forEach>
...
<nav style="background: dimgray">
    <ul>
        <c:if test="${requestScope.currentPage != 1}">
            <li>
                <a href="manager_serv?page=${requestScope.currentPage-1}"><%=bundle.getString("pagination.previous")%></a>
            </li>
        </c:if>
        ...
        <c:forEach begin="1" end="${requestScope.pagesCount}" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <li>
                        <a href="manager_serv?page=${i}">${i}</a>
                    </li>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        ...
        <c:if test="${requestScope.currentPage lt requestScope.pagesCount}">
            <li>
                <a href="manager_serv?page=${requestScope.currentPage+1}"><%=bundle.getString("pagination.next")%></a>
            </li>
        </c:if>
    </ul>
</nav>
